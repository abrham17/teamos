  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {tier.key === "pro" ? <Shield className="w-5 h-5 text-purple-400" /> : <Zap className="w-5 h-5 text-[var(--accent)]" />}
      </div>
      ...
      <div className="mt-8 space-y-6 flex-1">
         <div className="space-y-4">
            <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">
               <span className="flex items-center gap-2"><Users className="w-3 h-3"/> Active Seats</span>
               <span className="text-[var(--accent)] font-mono">{seats}</span>
            </div>
            <input
              type="range"
              min={1}
         ...
         <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">Performance Bandwidth</label>
            <select
              value={usageTier}
         ...
         <ul className="space-y-3 pt-4">
            {tier.features.map((f) => (
              <li key={f} className="flex gap-3 text-sm text-[var(--text-secondary)]">
                ...
                <span className="leading-tight font-medium text-xs uppercase tracking-tight">{f}</span>
              </li>
            ))}
         </ul>
      </div>
        ...
        {!isSignedIn ? (
          <SignUpButton mode="modal" forceRedirectUrl={redirectUrl}>
            ...
            >
              <span className="relative z-10 flex items-center justify-center gap-2">
                Launch Workspace <ArrowRight className="h-4 w-4 transition-transform group-hover/btn:translate-x-1" />
              </span>
            </button>
          </SignUpButton>
        ) : !currentTeamId ? (
          ...
          >
            <span className="relative z-10 flex items-center justify-center gap-2">
              ...
              {checkoutBusy ? "Initializing..." : "Upgrade Team"}
            </span>
          </button>
        )}
      </div>
...
export function HomePricing() {
    ...
    fetchBillingPlans()
      .then((c) => {
        ...
        for (const p of c.plans) {
          ...
          next[p.key] = { seats: 5, usage: "standard" };
        }
        setPrefs(next);
      })
  ...
  const handleCheckout = async (planKey: string) => {
    ...
    if (!q) {
      error("Price calculation pending...");
      return;
    }
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="text-center mb-16 space-y-4">
          ...
          <h2 id="home-pricing-heading" className="text-6xl md:text-7xl font-black uppercase tracking-tighter italic text-gradient leading-[0.85]">
            Unlimited <br/> Scaling.
          </h2>
          <p className="max-w-2xl mx-auto text-slate-400 text-lg">
            {catalog?.disclaimer ||
              "Simple per-user pricing with no base fees. Scale your team without hidden limits."}
          </p>
        </div>
        ...
        {catalogError || !catalog ? (
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3 items-start">
            {freePlan && (
              ...
              >
                ...
                <div className="flex items-baseline gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-[var(--text-dim)] uppercase tracking-widest">/ 2-mo trial</span>
                </div>
                <p className="text-sm text-[var(--text-secondary)] mb-8 leading-relaxed italic">
                  Explore the graph with a full 60-day window. Limited to 3 seats.
                </p>
                <ul className="space-y-3 flex-1">
                  {freePlan.features.map((f) => (
                    <li key={f} className="flex gap-3 text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">
            ...
            {paidPlans.map((tier) => {
              const pr = prefs[tier.key] || { seats: 5, usage: "standard" };
              return (
                <PaidTierCard
                  key={tier.key}
                  tier={tier}
        ...
        <div className="mt-20 flex flex-col md:flex-row items-center justify-between gap-8 p-8 rounded-3xl bg-white/[0.02] border border-white/5">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h4 className="font-bold text-white uppercase tracking-tight">Security & Scale</h4>
                 <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">Encrypted Knowledge Graphs // SOC2 Ready</p>
              </div>
           </div>
