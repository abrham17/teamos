...
function CitationLink({ c }: { c: ChatCitation }) {
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-2 px-3 py-1.5 border border-[var(--border-subtle)] bg-[var(--surface-1)] hover:bg-[var(--surface-2)] hover:border-[var(--accent)]/30 transition-colors duration-200 group"
    >
      {isPlan ? (
          <Layers className="h-3 w-3 text-[var(--warning)] transition-transform" />
      ) : (
          <FileText className="h-3 w-3 text-[var(--accent)] transition-transform" />
      )}
      ...
      {isPlan && c.source_kind && (
          <span className="text-[8px] font-black uppercase px-1.5 py-0.5 bg-[var(--warning)]/10 text-[var(--warning)] border border-[var(--warning)]/20">
              {c.source_kind}
          </span>
      )}
    </Link>
