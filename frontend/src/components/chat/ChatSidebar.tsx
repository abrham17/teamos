...
export function ChatSidebar({
  ...
  const sessionCls = (isActive: boolean) => {
    ...
    return [
      "flex items-center gap-3 px-3 py-2 w-full rounded-xl text-sm transition-colors duration-150 text-left group/sess",
      isActive
        ? "bg-[var(--user-bubble)] text-[var(--text-primary)] font-medium"
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "bg-[var(--chat-sidebar-bg)] border-r border-[var(--border-subtle)] flex flex-col h-full shrink-0 transition-[width] duration-[220ms] ease-in-out overflow-hidden",
      ...
      >
        ...
        <div className={cn("shrink-0", collapsed ? "py-3 flex justify-center" : "p-2 pt-3")}>
          ...
          <button
              onClick={onNewChat}
              className="flex items-center gap-2 w-full px-3 py-2 text-sm rounded-full border border-[var(--border-strong)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-1)] font-medium transition-colors"
            >
              <PlusCircle className="w-4 h-4" />
              New Chat
            </button>
            ...
            if (isRenaming && !collapsed) {
              return (
                <div key={s.id} className="flex items-center gap-1 px-2 py-1">
                  ...
                  />
                  <button onClick={confirmRename} className="p-1 rounded-lg text-[var(--accent)] hover:bg-[var(--surface-2)] transition-colors"><Check className="w-3.5 h-3.5" /></button>
                  <button onClick={cancelRename} className="p-1 rounded-lg text-[var(--text-dim)] hover:bg-[var(--surface-2)] transition-colors"><X className="w-3.5 h-3.5" /></button>
                </div>
              );
            }
